...
- Inherit from standard Godot node types (`Node2D`, `CharacterBody2D`, `Area2D`, `Resource`).
- Mark classes as `partial` for Godot's C# source generator.
...
## 2. Signals & Event Patterns
- Use Godot C# typed signals with the `[Signal]` attribute:
```csharp
[Signal]
...
- Use pure C# events in domain services that do not require Godot editor binding.

## 3. Resource-Driven Architecture (`.tres`)
Define custom data schemas by inheriting from `Resource` and decorating with `[GlobalClass]`:
...
3. Use `Vector2.MoveToward` and `Mathf.MoveToward` for physics interpolation.
4. Use `StringName` constants for input actions.
5. All documentation and comments must remain technical and devoid of emojis or conversational filler.
